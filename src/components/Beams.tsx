...
const Beams = ({
  ...
  vec3 getNormal(vec3 pos) {
    ...
    gl_FragColor.rgb -= randomNoise / 15. * uNoiseIntensity;`
        ...
        material: { fog: true },
        uniforms: {
          diffuse: new THREE.Color(...hexToNormalizedRGB('#000000') as [number, number, number]),
...
const MergedPlanes = forwardRef(({ material, width, count, height }: { material: THREE.ShaderMaterial; width: number; count: number; height: number }, ref: any) => {
...
const PlaneNoise = forwardRef((props: { material: THREE.ShaderMaterial; width: number; count: number; height: number }, ref: any) => (
...
const DirLight = ({ position, color }: { position: [number, number, number]; color: string }) => {
  ...
  useEffect(() => {
    ...
    if (!cam) return;
    (cam as THREE.OrthographicCamera).top = 24;
    (cam as THREE.OrthographicCamera).bottom = -24;
    (cam as THREE.OrthographicCamera).left = -24;
    (cam as THREE.OrthographicCamera).right = 24;
    (cam as THREE.OrthographicCamera).far = 64;
